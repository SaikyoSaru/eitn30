...
FileSystem::FileSystem() : savedPageLen(0) {
  int slask;
...
bool FileSystem::writeFile(char *path, char *name, byte *theData,
                           udword theLength) {
  cout << "path: " << path << " name: " << name << endl;
  if (strncmp(path, "private", 7) == 0 &&
      strncmp(name, "private.htm", 11) == 0) {
    delete[] savedPage;
...
byte *FileSystem::readFile(char *path, char *name, udword &theLength) {
  cout << "path: " << path << " name: " << name << endl;
  if (strncmp(path, "dynamic", 7) == 0 &&
      strncmp(name, "dynamic.htm", 11) == 0) {
    cout << "found it" << endl;
